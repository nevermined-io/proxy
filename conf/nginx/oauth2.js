...
function __ORIGINAL__introspectAccessToken(r) {
    ...
    r.log("OAuth sending introspection: ")
    r.log(" token: " + r.variables.access_token)
    r.log(" header: " + authHeader)
    r.subrequest("/_oauth2_send_introspection_request", "token=" + r.variables.access_token + "&authorization=" + authHeader,
        function(reply) {
            if (reply.status != 200) {
                r.error(`OAuth unexpected response from authorization server: ${JSON.stringify(reply)}`);
                r.return(401);
            }

            // We have a response from authorization server, validate it has expected JSON schema
            try {
                r.log(`OAuth token introspection response: ${JSON.stringify(reply)}`)
